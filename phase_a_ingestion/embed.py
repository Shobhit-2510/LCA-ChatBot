...
# BGEEmbeddings is inherited from HuggingFaceEmbeddings but overrides embed_query to prepend the BGE instruction.
class BGEEmbeddings(HuggingFaceEmbeddings):
    ...
    def embed_query(self, text: str) -> list[float]:  # Prepend instruction to queries only
        # super() calls HuggingFaceEmbeddings.embed_query(), which encodes the text and normalizes it.
        return super().embed_query(_BGE_QUERY_INSTRUCTION + text)


@lru_cache(maxsize=1) # Cache(stores) the functions result for 1 parameter value, so subsequent calls with the same parameter return the cached result instead of recomputing it.
def get_embeddings() -> BGEEmbeddings:
    ...
    return BGEEmbeddings(
        ...
        encode_kwargs={"normalize_embeddings": True},  # L2-normalize document embeddings when building the vector database
        query_encode_kwargs={"normalize_embeddings": True},  # L2-normalize queries when retrieving from the vector database
    )
